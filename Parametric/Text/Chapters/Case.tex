The aim of this section is to show more in detail the calculation procedure. The methodology is applied to a basic power system such as the 5-bus system from the GridCal tutorial \cite{gridcal}, shown in Fig. \ref{fig:5bus}. 
...
\begin{figure}[!htb]\centering
  ...
  \label{fig:5bus}
\end{figure}
The system is formed by four PQ buses and one slack bus, corresponding to node 1. Lines are originally modeled with a $\pi$ equivalent. Nevertheless, the parallel capacitances are removed in order to have voltages below the level imposed by the slack bus, which should help at obtaining a more intuitive result. 
