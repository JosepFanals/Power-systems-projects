...
for i in range(len(mat_inv)):
    for j in range(len(mat_inv)):
        if mat_inv[i, j] < 1e-10 and mat_inv[i, j] > -1e-10:
            mat_inv[i, j] = 0
...
for i in range(len(rhs0)):
    if i in pq0:
        ...
        rhs1[i] = np.imag(Pac[i] * Xac[cc - 1, i] - 1j * conv1x(Xac, Qac, i, trobats_pv, 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
        trobats_pv += 1
    elif i in pqv0:
        rhs1[i] = np.imag((Pac[i] - 1j * Qac[i]) * Xac[cc - 1, i] - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
...
for i in range(len(df_busos_AC)):
    if df_busos_AC.iloc[i, 4] == "PV" or df_busos_AC.iloc[i, 4] == "PQV":
        rhs2[trobats_pqv_pv] = df_busos_AC.iloc[i, 3] ** 2
        trobats_pqv_pv += 1
...
for i in range(len(df_convertidors)):
    if not np.isnan(df_convertidors.iloc[i, 3]):
        rhs7[trobats] = df_convertidors.iloc[i, 3] ** 2
        trobats += 1
    ...
    for i in range(len(rhs0)):  # partir en 2 opcions, si connecta amb conv. on es controla la Pf o no. S'hauran d'afegir convolucions
        ...
        if i in bac_pf:
            ...
            if i in pq0:
                ...
                # rhs0[i] = np.real(Pac[i] * Xac[cc - 1, i] - 1j * conv1x(Xac, Qac, i, trobats_pv, 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                rhs0[i] = g_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) - g_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) + np.real(Pac[i] * Xac[cc - 1, i] - 1j * conv1(Xac[:, i], Qac[:, trobats_pv], 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                trobats_pv += 1
            elif i in pqv0:
                rhs0[i] = g_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) - g_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) + np.real((Pac[i] - 1j * Qac[i]) * Xac[cc - 1, i] - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
            ...
            if i in pq0:
                ...
                rhs0[i] = np.real(Pac[i] * Xac[cc - 1, i] - 1j * conv1(Xac[:, i], Qac[:, trobats_pv], 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                trobats_pv += 1
            elif i in pqv0:
                rhs0[i] = np.real((Pac[i] - 1j * Qac[i]) * Xac[cc - 1, i] - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
    ...
    for i in range(len(rhs1)):  # partir en 2 opcions, si connecta amb conv. on es controla la Pf o no. S'hauran d'afegir convolucions
        ...
        if i in bac_pf:
            ...
            if i in pq0:
                ...
                # rhs0[i] = np.real(Pac[i] * Xac[cc - 1, i] - 1j * conv1x(Xac, Qac, i, trobats_pv, 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                rhs1[i] = g_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) + b_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) + g_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) + np.imag(Pac[i] * Xac[cc - 1, i] - 1j * conv1(Xac[:, i], Qac[:, trobats_pv], 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                trobats_pv += 1
            elif i in pqv0:
                rhs1[i] = g_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) + b_conv[ind_convE[tr]] * conv1(Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_re[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) + g_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, cc-1, 0, cc) + g_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_re[:, tr], 1, 0, cc-1, cc, 0, cc) - b_conv[ind_convE[tr]] * conv1(Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, cc-1, 0, cc) - b_conv[ind_convE[tr]] * conv2(M[:, ind_convE[tr]], Vdc_im[:, bdc_pf[tr]], E_im[:, tr], 1, 0, cc-1, cc, 0, cc) + np.imag((Pac[i] - 1j * Qac[i]) * Xac[cc - 1, i] - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
            ...
            if i in pq0:
                ...
                rhs1[i] = np.imag(Pac[i] * Xac[cc - 1, i] - 1j * conv1(Xac[:, i], Qac[:, trobats_pv], 1, cc-1, cc, 0)  - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
                trobats_pv += 1
            elif i in pqv0:
                rhs1[i] = np.imag((Pac[i] - 1j * Qac[i]) * Xac[cc - 1, i] - Ysl[i] * V0[cc] - Y_sh[i] * Vac[cc - 1, i])
